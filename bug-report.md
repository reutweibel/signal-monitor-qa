...
**Actual result:** Both requests succeed and return **different `jobId`s**, each running independently against the same case data. Verified via API: two calls to `POST /cases/case-6/enrich` returned `jobId: 9def3284...` and `jobId: 3cafa662...`; `GET /cases/case-6/enrichment-status` (which the frontend polls) only ever exposes the *latest* job: the first job's progress and completion are never surfaced anywhere in the UI. Yet both jobs completed and both wrote real results: `phone-4` ended up with **two separate `CarrierLookup` entries** and `social-5` with **two separate `SocialGraph` entries**, from two runs the user only initiated (and only ever saw progress for) once.
...
**Additional evidence (manually reproduced through the live UI on `case-14`, "Operation Pale Horizon"):** starting from a clean case (0 enrichments on all items), clicking "Run Enrichment" 3 times in quick succession, each click landing while the previous job was still running since nothing on screen indicates a job is in progress, produced **3 separate jobs** (confirmed in backend logs: `Starting enrichment job ... for case case-14` logged three times) and **3 enrichment entries on every single item**. Critically, the 3 entries are not identical copies: they're 3 independently-simulated results that can directly contradict each other:
